...
\begin{figure}[h!]
    ...
    \caption{Conexiónes del motor driver shield L293D}
...
El servomotor es representado a través del \textit{struct} \texttt{servoMotor}, y se controla con la librería estándar de Arduino \texttt{Servo}. El método \texttt{setServoAngle()} es el responsable de rotar el servo a un ángulo específico.
...
\begin{lstlisting}[caption={Flujo principal de control en Arduino}, style=pseudocodigo]
inicializar_robot()
...
El flujo de funcionamiento apreciado en al figura \ref{fig:diag_fin} es el siguiente: al recibir la \textbf{wake word}, el cliente comienza a grabar la voz del usuario, enviando fragmentos de audio al servidor mediante un evento especifico. Finalizada la grabacion se procesa el audio para obtener la transcripcion del mensaje mediante whisper. Posteriormente el texto resultante se procesa mediante Piper para obtener la respuesta en audio, la cual es enviada junto al texto al cliente para su reproduccion.
...
\begin{figure}[H]
    ...
    \caption{Diagrama de comunicacion final Servidor-Cliente}
    \label{fig:diag_fin}
\end{figure}
...
Para la detección de la palabra clave se utiliza la librería \texttt{Porcupine}, que permite reconocer localmente una \textit{wake word} predefinida sin necesidad de conexión a Internet. El cliente inicializa \texttt{Porcupine} con una clave de acceso (\texttt{ACCESS\_KEY}) y un modelo de palabra clave, y escucha de manera continua el micrófono. Cuando la salida del modelo indica que la palabra ha sido detectada, el sistema reproduce un sonido breve de inicio y pasa al modo de grabación.
...
\begin{lstlisting}[caption={Recepción de comandos seriales en Arduino},style=pseudocodigo]
...
\begin{lstlisting}[caption={Envio de comandos seriales en Raspberry},style=pseudocodigo]
...
\begin{figure}[h]
    ...
    \caption{Boceto inicial hecho por Iván Mansilla}
